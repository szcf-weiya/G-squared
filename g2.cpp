...
Rcpp::List g2(Rcpp::DataFrame ds)
{
  ...
  size_t *order = (size_t*)malloc(sizeof(size_t)*x.size());
  ...
  gsl_vector_scale(y, gsl_stats_sd(y->data, 1, n));
  ...
  for(size_t i = m - 1; i < n; i++){
    ...
    gsl_vector *mi = gsl_vector_alloc(i-2*m+1);
    // construct k
    ...
    for (size_t kk = 0; kk < i-2*m+2; kk++){
      ...
      gsl_vector_add_constant(xx, gsl_stats_mean(xx->data, 1, i-k[kk]+1));
    ...
    Mi = gsl_vector_max(mi);
    Bi[i] = bi;
    Ti[i] = ti;
  }
  // step final result
  return(Rcpp::List::create(
    Rcpp::Named("g2m") = 1-exp(-2./n*(Mi[n-1]-lambda)),
