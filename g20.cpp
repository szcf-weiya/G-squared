...
int genXY(const int n, const double epsi, const int type, const int resimulate, double *x, double *y)
{
  gsl_rng *r;
  gsl_rng_env_setup();
  ...
  for (size_t i = 0; i < num_noise; i++)
  {
    ...
    for (size_t j = 0; j < num_type; j++)
    {
      ...
      for (size_t k = 0; k < n2; k++)
      {
        double *x = new double[n];
        double *y = new double[n];
        genXY(n, noise, j, 0, x, y);
        // correlation
        val_cor2[k] = pow(gsl_stats_correlation(x, 1, y, 1, n), 2);
        // g2
        g2(x, y, n, val_g2m2+k, val_g2t2+k);
        cout << val_g2m2[k] << " " <<val_g2t2[k] << endl;
        delete [] x;
        delete [] y;
      }
